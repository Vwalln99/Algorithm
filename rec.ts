// 5!=120
function fak(num: number): number {
    let result = 1;
    for (let i = 2; i <= num; i++) {
        result *= i;
    }
...
function numInArr(arr: any[], num: number): boolean {
    return arr.some((element) => { //some ist abbruchbedingung
...
function oddNumRec(numbers: number[]) {

    const oddNum: number[] = [];
    function helper(numbers: number[]) {
        if (numbers.length === 0) return oddNum;
        if (numbers[0] % 2 !== 0) oddNum.push(numbers[0]);
...
function stringPalindrome(text: string): boolean {
    if (text.length <= 1) {
        return true;
